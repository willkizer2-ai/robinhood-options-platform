...
function PerformanceScreen() {
  ...
  if (d.empty) return <Panel eyebrow="Performance" title="No trade history yet"><p style={{ margin: 0, color: 'var(--text-muted)', fontSize: 13 }}>Performance populates once real trade history is connected.</p></Panel>;
  const max = Math.max(31, ...d.months.map((m) => Math.abs(m.r)));
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}><SourceTag live={d.live} /></div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px,1fr))', gap: 12 }}>
        {d.stats.map((s, i) => <StatTile key={i} label={s.label} value={s.value} suffix={s.suffix} tone={s.tone} delta={s.delta} />)}
      </div>
      <Panel eyebrow={d.name || 'Strategy'} title="Monthly Returns" action={<Badge tone="up" variant="soft">Backtested</Badge>}>
        {d.months.length === 0
          ? <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: 13 }}>No monthly data available.</p>
          : <div style={{ display: 'flex', alignItems: 'flex-end', gap: 14, height: 180, padding: '8px 4px' }}>
            {d.months.map((mo, i) => {
              const up = mo.r >= 0; const h = (Math.abs(mo.r) / max) * 130 + 6;
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: 8, height: '100%' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, color: up ? 'var(--up-text)' : 'var(--down-text)' }}>{up ? '+' : ''}{mo.r}%</span>
                  <div style={{ width: '100%', maxWidth: 38, height: h, borderRadius: 'var(--radius-sm)', background: up ? 'var(--up)' : 'var(--down)', boxShadow: up ? 'var(--glow-up)' : 'var(--glow-down)', opacity: 0.92 }} />
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-faint)' }}>{mo.m}</span>
                </div>
              );
            })}
          </div>}
        <p style={{ margin: '10px 2px 0', fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--text-faint)' }}>Past performance is not indicative of future results. Figures shown are backtested.</p>
      </Panel>
    </div>
  );
}
...
function Desk() {
  ...
  return (
    <div className="shell">
      ...
      <div className="stickytabs"><div style={{ padding: '0 24px' }}><Tabs value={tab} onChange={setTab} tabs={tabs} style={{ border: 'none' }} /></div></div>
      <div className="main">
